...
export default function OccDashboard() {
  ...
  return <main className="shell">
    <header className="topbar"><div><p className="eyebrow">Emma OCC v4.3</p><h1>Personal Operations Control Center</h1><p className="subtitle">Roster-first commute execution with NS, Google walking, weather, and backup routing.</p></div><div className="mission-cluster"><StatusBadge value={snapshot.risk}/><StatusBadge value={snapshot.direction === 'none' ? 'NO COMMUTE' : snapshot.direction.toUpperCase()}/><button className="refresh" onClick={() => void reload()} disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''}/> Reload</button></div></header>
    ...
    <section className="summary-grid">
      ...
      <article className="panel metric"><div className="panel-title"><TrainFront size={18}/> NS train</div><h2>{snapshot.train.cancelled ? 'Cancelled' : `${snapshot.train.departure} → ${snapshot.train.arrival}`}</h2><p>{snapshot.train.direct ? 'Direct · 0 transfers' : `${snapshot.train.transfers ?? 1} transfer(s)`}{snapshot.train.platform ? ` · platform ${snapshot.train.platform}` : ''}{snapshot.train.arrivalPlatform ? ` → ${snapshot.train.arrivalPlatform}` : ''}</p></article>
    ...
    <section className="content-grid">
      ...
      <article className="panel"><div className="panel-title"><MapPin size={18}/> Google walking</div><h2>{snapshot.walking.first.durationMinutes + snapshot.walking.last.durationMinutes} min total</h2><p>First leg: {snapshot.walking.first.durationMinutes} min · {formatDistance(snapshot.walking.first.distanceMeters)}</p><p>Last leg: {snapshot.walking.last.durationMinutes} min · {formatDistance(snapshot.walking.last.distanceMeters)}</p></article>
      ...
      <article className="panel"><div className="panel-title"><BellRing size={18}/> Operational watch</div><ul className="watch-list"><li><CheckCircle2 size={16}/> Notify only in the 60–90 minute action window</li><li><CheckCircle2 size={16}/> Delay, cancellation, or platform change</li><li><CheckCircle2 size={16}/> Weather or walking-route penalty</li><li><CheckCircle2 size={16}/> NS first; 9292 backup</li></ul></article>
