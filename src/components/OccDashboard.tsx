...
export default function OccDashboard() {
  ...
  return <main className="shell">
    <header className="topbar"><div><p className="eyebrow">Emma OCC v4.1</p><h1>Personal Operations Control Center</h1><p className="subtitle">Live-ready roster, walking, rail, weather, email, and decision intelligence.</p></div><div className="mission-cluster"><StatusBadge value={snapshot.risk}/><StatusBadge value={snapshot.mission}/><button className="refresh" onClick={() => void reload()} disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''}/> Reload</button></div></header>
    ...
    <section className="content-grid">
      ...
      <article className="panel wide"><div className="panel-title"><Wifi size={18}/> Integration control</div><div className="integration-list">{snapshot.integrations.map((item) => <div className="integration-row" key={item.name}>{item.source === 'live' ? <Wifi size={16}/> : <WifiOff size={16}/>}<span><b>{item.name}</b><small>{item.message}</small></span><StatusBadge value={item.source.toUpperCase()}/></div>)}</div></article>
